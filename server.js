...
function gateKeeper(req, res, next) {
  ...
  // ^ create new object w/ null as default values,
  // if `user` and `pass` values found in parsed headers, use those instead of default
  req.user = USERS.find(
    (usr, index) => usr.userName === user && usr.password === pass
  );
  //^ if user in `USERS` with username and password from request headers,
  // set `req.user` equal to that object. Otherwise, `req.user` is undefined
  next();
}
